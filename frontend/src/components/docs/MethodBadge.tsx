...
export function StatusBadge({ code }: { code: string }) {
  ...
  if (num >= 200 && num < 300)
    color = 'bg-emerald-500/[0.12] text-emerald-400 border-emerald-500/40';
  else if (num >= 300 && num < 400) color = 'bg-cyan/[0.12] text-cyan border-cyan/40';
  else if (num >= 400 && num < 500)
    color = 'bg-amber-400/[0.12] text-amber-400 border-amber-400/40';
  else if (num >= 500) color = 'bg-rose-500/[0.12] text-rose-400 border-rose-500/40';
  return (
    <span
      className={
        'inline-flex h-6 items-center rounded border px-2 font-mono text-xs font-semibold ' + color
      }
    >
      {code}
    </span>
